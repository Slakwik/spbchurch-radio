...
### v4.1 — Журнал приложения
Встроенный журнал событий для отслеживания ошибок прямо в приложении, без подключения Xcode.

- **`LogManager`** (`@MainActor`, singleton) — буфер событий до 500 записей с уровнями `info` / `warning` / `error`. Персистится в `Documents/app_log.json`. По умолчанию включён, можно выключить
- **`LogsView`** — полноэкранный экран в Настройках («Диагностика › Журнал приложения») с:
  - тумблером «Журналирование включено/выключено» (сохраняется в `UserDefaults`)
  - сегментным фильтром по уровню (Все / Инфо / Внимание / Ошибка)
  - кнопкой удаления с подтверждением через `.alert(role: .destructive)`
  - ShareLink-экспортом в текстовый файл (имя `spbchurch-log-<timestamp>.txt`) — можно отправить разработчику через AirDrop / Mail
  - long-press на записи → «Скопировать»
  - empty state с подсказкой
- **Логирование подключено** в ключевые сервисы:
  - `DownloadManager` — старт / завершение / отмена / ошибки загрузки и удаления
  - `RadioStreamService` — старт/стоп эфира, ошибки audio session, смена текущего трека
  - `FilePlayerService` — воспроизведение трека (с указанием поток / локальный файл)
  - `TrackListService` — успех/ошибка загрузки каталога с количеством треков
- В DEBUG-сборках записи дополнительно дублируются в Xcode console через `print`
- Версия в about-экранах поднята до 4.1

### v4.0.4 — Возврат к ephemeral + Connection: close
v4.0.3 пробовал переключиться на default URLSession + reuse соединений в расчёте на HTTP/2. На практике ожидание перед следующей загрузкой вернулось — keep-alive socket пула по-прежнему задерживает новый запрос. Откатываю клиент к рабочей конфигурации:

- `URLSessionConfiguration.ephemeral` снова — никакого общего пула, чистое соединение на каждую загрузку
- `Connection: close` снова в `URLRequest` — сервер закрывает сокет сразу после ответа
- nginx-конфиг с HTTP/2 в `server/nginx/stream.conf` остаётся — он не вредит и пригодится для других клиентов (браузер, Safari), даже если наше приложение его не использует
